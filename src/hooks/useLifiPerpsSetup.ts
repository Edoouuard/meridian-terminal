...
import { ActionType, getPerpsClient, type LifiPerpsProviderId } from "@/lib/integrations/lifiPerps";
...
export function useLifiPerpsSetup(provider: LifiPerpsProviderId): UseLifiPerpsSetupResult {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const setup = await client.checkSetup({ provider, address });
      setPendingSteps(setup.setup);
      let depositFlow: DepositFlow | undefined;
      if (!setup.isReady) {
        try {
          depositFlow = await client.getDepositFlow({ provider, address });
        } catch {
  ...
  }, [isConnected, address, provider, client]);

  const runNextStep = useCallback(async () => {
    if (!address || pendingSteps.length === 0 || runningStep) return;
    setRunningStep(true);
    try {
      await client.executeProviderSetupAction({ provider, address, step: pendingSteps[0] });
      await refresh();
    } catch (err) {
  ...
  }, [client, address, provider, pendingSteps, runningStep, refresh]);
  ...
  const deposit = useCallback(
    ...
    [client, address, provider, state.depositFlow, refresh],
  );
